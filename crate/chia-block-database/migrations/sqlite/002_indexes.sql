-- =============================================================================
-- Migration 002: Performance Indexes (SQLite)
-- =============================================================================
...
-- SQLite doesn't support INCLUDE, so we use a composite index
CREATE INDEX IF NOT EXISTS idx_coins_puzzle_hash_covering ON coins(puzzle_hash, coin_id, amount, created_block, parent_coin_info);
...
CREATE INDEX IF NOT EXISTS idx_coins_balance_calc_covering ON coins(puzzle_hash, amount, created_block, coin_id, parent_coin_info);
...
-- SQLite doesn't support WHERE clauses in CREATE INDEX the same way, so we use regular indexes
...
-- Optimized index for get_common_puzzles pagination
...
CREATE INDEX IF NOT EXISTS idx_spends_coin_id_covering ON spends(coin_id, spent_block);
...
CREATE INDEX IF NOT EXISTS idx_spends_analytics_covering ON spends(puzzle_id, solution_id, spent_block, coin_id);

-- Performance index for puzzle usage frequency analysis  
CREATE INDEX IF NOT EXISTS idx_spends_puzzle_frequency ON spends(puzzle_id, coin_id, spent_block);

-- Performance index for solution usage frequency analysis  
CREATE INDEX IF NOT EXISTS idx_spends_solution_frequency ON spends(solution_id, coin_id, spent_block);
