use crate::error::{PeerTlsError, Result};
...
fn try_load_existing_cert() -> Result<ChiaCertificate> {
    ...
    let private_key = std::fs::read(&private_key_path)
        .map_err(|e| PeerTlsError::Certificate(format!("Failed to read private key: {}", e)))?;

    // Try to read certificate
    let certificate = std::fs::read(&cert_path)
        .map_err(|e| PeerTlsError::Certificate(format!("Failed to read certificate: {}", e)))?;
...
fn generate_self_signed_cert(_private_key: &[u8]) -> Result<Vec<u8>> {
    // This is a simplified implementation for development
    ...
    if let Some(start) = pem_str.find(start_marker) {
        if let Some(end) = pem_str.find(end_marker) {
            ...
            return base64_decode(&cert_content)
                .map_err(|e| PeerTlsError::Certificate(format!("Failed to decode certificate: {}", e)));
        }
    }
    
    Err(PeerTlsError::Certificate("Invalid PEM format".to_string()))
}
...
pub fn create_tls_connector(cert: &ChiaCertificate) -> Result<native_tls::TlsConnector> {
    ...
    let connector = native_tls::TlsConnector::builder()
        ...
        .build()
        .map_err(|e| PeerTlsError::Tls(e.to_string()))?;
