...
fn try_load_existing_cert() -> Result<ChiaCertificate, ChiaError> {
    ...
    let private_key = std::fs::read(&private_key_path)
        .map_err(|e| ChiaError::Tls(format!("Failed to read private key: {}", e)))?;

    // Try to read certificate
    let certificate = std::fs::read(&cert_path)
        .map_err(|e| ChiaError::Tls(format!("Failed to read certificate: {}", e)))?;
...
fn generate_cert() -> Result<ChiaCertificate, ChiaError> {
...
fn generate_private_key() -> Result<Vec<u8>, ChiaError> {
    // This is a simplified implementation for development
    // In practice, you'd use proper cryptographic libraries
    let key: Vec<u8> = (0..32).map(|i| (i as u8) ^ 0xAA).collect();
...
fn generate_self_signed_cert(_private_key: &[u8]) -> Result<Vec<u8>, ChiaError> {
    // This is a simplified implementation for development
    // In practice, you'd use proper certificate generation libraries
    let cert_data: Vec<u8> = (0..64).map(|i| (i as u8) ^ 0x55).collect();
...
fn parse_pem_to_der(pem_data: &[u8]) -> Result<Vec<u8>, ChiaError> {
    ...
    if let Some(start) = pem_str.find(start_marker) {
        if let Some(end) = pem_str.find(end_marker) {
            ...
            return base64_decode(&cert_content)
                .map_err(|e| ChiaError::Tls(format!("Failed to decode certificate: {}", e)));
        }
    }
    
    Err(ChiaError::Tls("Invalid PEM format".to_string()))
}

/// Create a TLS connector from the certificate
pub fn create_tls_connector(_cert: &ChiaCertificate) -> Result<native_tls::TlsConnector, ChiaError> {
...
pub fn create_ws_connector(cert: &ChiaCertificate) -> Result<Connector, ChiaError> {
...
pub fn calculate_node_id(cert_der: &[u8]) -> Result<String, ChiaError> {
...
fn base64_decode(data: &str) -> Result<Vec<u8>, ChiaError> {
...
pub fn save_cert_to_file<P: AsRef<Path>>(cert: &ChiaCertificate, dir: P) -> Result<(), ChiaError> {
    let dir = dir.as_ref();
